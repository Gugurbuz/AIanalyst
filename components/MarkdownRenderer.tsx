...
const parseMarkdown = (text: string, highlightedLines: number[], rephrasingText: string | null, highlightedUserSelectionText: string | null): string => {
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (isTableLine && isHeaderSeparator) {
            ...
            html += `<div class="overflow-x-auto my-5"><table class="w-full text-sm border-collapse border border-slate-300 dark:border-slate-600 table-fixed">
                        <thead class="bg-slate-100 dark:bg-slate-800">
                            <tr${isHighlighted ? ' class="highlight-new"' : ''}>${headers.map(h => `<th class="p-3 font-semibold text-left border border-slate-300 dark:border-slate-600">${processInline(h)}</th>`).join('')}</tr>
                        </thead>
                        <tbody>`;
            inTable = true;
            i++; 
        ...
        if (inTable) {
            if (isTableLine) {
                ...
                const rowClass = `border-t border-slate-200 dark:border-slate-700 even:bg-slate-50 dark:even:bg-slate-800/50 ${isHighlighted ? 'highlight-new' : ''}`;
                html += `<tr class="${rowClass.trim()}">${cells.map(c => `<td class="p-3 border border-slate-300 dark:border-slate-600 break-words">${processInline(c)}</td>`).join('')}</tr>`;
                continue;
            } else {
