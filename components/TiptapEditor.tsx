...
export const TiptapEditor: React.FC<TiptapEditorProps> = ({ content, onChange, onSelectionUpdate }) => {
    const editor = useEditor({
        ...
        editorProps: {
            attributes: {
                class: 'prose prose-slate dark:prose-invert max-w-none focus:outline-none p-4 md:p-6 h-full',
            },
        },
    });
    ...
    React.useEffect(() => {
        if (editor && !editor.isDestroyed) {
            ...
            if (currentContentAsMarkdown !== content) {
                // Parse the markdown to HTML, sanitize it, then set the content
                const html = marked.parse(content || '');
                const sanitizedHtml = DOMPurify.sanitize(html);
                editor.commands.setContent(sanitizedHtml, false); // false prevents firing onUpdate again
            }
        }
    }, [content, editor]);
