...
interface ExportDropdownProps {
    ...
    filename: string;
    isTable?: boolean;
    isVisualization?: boolean; // This prop now indicates if content is structured Markdown for visualization
}
...
export const ExportDropdown: React.FC<ExportDropdownProps> = ({ content, filename, isTable = false, isVisualization = false }) => {
    ...
    const handleExport = (format: 'pdf' | 'md' | 'svg' | 'mmd') => {
        if (format === 'svg') {
            const svgElement = document.getElementById('mermaid-diagram-container')?.querySelector('svg');
             if (svgElement) {
                // Ensure xmlns is present for proper rendering as standalone file
    ...
    return (
        <div className="relative" ref={dropdownRef}>
            ...
            {isOpen && (
                <div className="origin-top-right absolute right-0 mt-2 w-full sm:w-48 min-w-0 rounded-md shadow-lg bg-white dark:bg-slate-800 ring-1 ring-black ring-opacity-5 focus:outline-none z-10"> {/* Adjusted width for responsiveness */}
