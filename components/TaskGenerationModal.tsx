...
import type { Conversation, Task, TaskSuggestion } from '../types';
...
export const TaskGenerationModal: React.FC<TaskGenerationModalProps> = ({ isOpen, onClose, conversation, isGenerating, setIsGenerating }) => {
    ...
    useEffect(() => {
        if (isOpen && conversation.generatedDocs.analysisDoc) {
            const generate = async () => {
                ...
                setSuggestions([]);
                try {
                    const result = await geminiService.generateTasksFromAnalysis(conversation.generatedDocs.analysisDoc);
    ...
    }, [isOpen, conversation.generatedDocs.analysisDoc]);
    ...
    return (
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4" onClick={onClose}>
            <div className="bg-white dark:bg-slate-800 rounded-lg shadow-2xl w-full max-w-3xl" onClick={e => e.stopPropagation()}>
                ...
                <footer className="flex justify-between items-center p-4 border-t border-slate-200 dark:border-slate-700">
                    ...
                    <div className="flex gap-3">
                        ...
                        <button 
                            ...
                            className="px-4 py-2 text-sm font-medium text-white bg-sky-600 rounded-md shadow-sm hover:bg-sky-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-sky-500 transition disabled:opacity-50 flex items-center"
                        >
