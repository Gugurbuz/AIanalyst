...
interface ShareModalProps {
    ...
    onUpdateShareSettings: (conversationId: string, is_shared: boolean) => Promise<void>;
}
...
export const ShareModal: React.FC<ShareModalProps> = ({ isOpen, onClose, conversation, onUpdateShareSettings }) => {
    ...
    const handleToggleSharing = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        setIsLoading(true);
        await onUpdateShareSettings(conversation.id, newIsShared);
    ...
    return (
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4" onClick={onClose}>
            <div 
                className="bg-white dark:bg-slate-800 rounded-lg shadow-2xl w-full max-w-lg" 
                    ...
                    {isShared && conversation.share_id && (
                        <div className="pt-4 space-y-2 animate-fade-in-up" style={{animationDuration: '0.3s'}}>
                            ...
                            <div className="flex gap-2">
                                ...
                                <button
                                    onClick={handleCopy}
                                    className="px-4 py-2 text-sm font-semibold text-white bg-sky-600 rounded-md shadow-sm hover:bg-sky-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-sky-500 transition w-32"
                                >
