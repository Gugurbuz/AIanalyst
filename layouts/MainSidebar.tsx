...
export const MainSidebar: React.FC<MainSidebarProps> = ({ user, profile, theme, onThemeChange, onLogout, onOpenShareModal }) => {
    ...
    return (
        <nav className={`bg-slate-50 dark:bg-slate-800 border-r border-slate-200 dark:border-slate-700 flex flex-col flex-shrink-0 transition-all duration-300 ease-in-out ${isSidebarOpen ? 'w-80' : 'w-20'}`}>
            {/* Header */}
            <div className={`flex items-center h-16 px-4 border-b border-slate-200 dark:border-slate-700 flex-shrink-0 ${!isSidebarOpen && 'justify-center'}`}>
                 <a href="/" className="flex items-center gap-3" aria-label="Asisty.AI Ana Sayfa">
                    <LogoIcon className="h-8 w-8 flex-shrink-0" />
                    {isSidebarOpen && <span className="text-xl font-bold text-slate-800 dark:text-slate-200">Asisty.AI</span>}
                </a>
            </div>
            ...
            {/* New Conversation and View Switcher */}
             <div className="p-2 space-y-2 border-b border-slate-200 dark:border-slate-700">
                <button
                    ...
                    className={`w-full h-11 flex items-center gap-2 rounded-lg transition-colors duration-200 text-white bg-indigo-600 hover:bg-indigo-700 shadow-sm ${isSidebarOpen ? 'px-3 justify-start' : 'justify-center'}`}
                >
                    <Pencil className="h-5 w-5 flex-shrink-0" />
                    {isSidebarOpen && <span className="text-sm font-semibold">Yeni Analiz</span>}
                </button>
                <div className="flex items-center gap-2">
                    <button
                        onClick={() => setAppMode('analyst')}
                        title="Analizler"
                        className={`w-full h-10 flex items-center gap-2 rounded-md text-sm font-semibold transition-colors ${appMode === 'analyst' ? 'bg-slate-200 dark:bg-slate-700 text-slate-800 dark:text-slate-100' : 'text-slate-500 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-700/50'} ${isSidebarOpen ? 'px-3 justify-start' : 'justify-center'}`}
                    >
                        <MessageSquare className="h-5 w-5 flex-shrink-0" />
                        {isSidebarOpen && 'Analizler'}
                    </button>
                    <button
                        onClick={() => setAppMode('backlog')}
                        title="Backlog"
                        className={`w-full h-10 flex items-center gap-2 rounded-md text-sm font-semibold transition-colors ${appMode === 'backlog' ? 'bg-slate-200 dark:bg-slate-700 text-slate-800 dark:text-slate-100' : 'text-slate-500 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-700/50'} ${isSidebarOpen ? 'px-3 justify-start' : 'justify-center'}`}
                    >
                        <ClipboardList className="h-5 w-5 flex-shrink-0" />
                        {isSidebarOpen && 'Backlog'}
                    </button>
                </div>
            </div>
            ...
            <div className="flex-1 flex flex-col overflow-hidden">
                ...
                <div className="flex-1 overflow-y-auto p-2 space-y-1">
                    {filteredConversations.map(conv => (
                         <div key={conv.id} className="relative group">
                            {editingId === conv.id && isSidebarOpen ? (
                                ...
                                <>
                                    <button onClick={() => setActiveConversationId(conv.id)} className={`w-full text-left rounded-md text-sm truncate transition-colors duration-150 ${conv.id === activeConversationId ? 'bg-indigo-100 dark:bg-indigo-900/50 text-indigo-700 dark:text-indigo-200 font-semibold' : 'text-slate-700 dark:text-slate-300 hover:bg-slate-200 dark:hover:bg-slate-700'} ${isSidebarOpen ? 'pl-3 pr-16 py-2.5' : 'py-2.5 flex justify-center'}`} title={conv.title}>
                                        {isSidebarOpen ? conv.title : <MessageSquare className="h-5 w-5" />}
                                    </button>
                                     {isSidebarOpen && (
             ...
             {/* Footer */}
             <div className="p-2 mt-auto flex-shrink-0 border-t border-slate-200 dark:border-slate-700">
                {isSidebarOpen && profile && (
                    <div className="px-2 py-3">
                        <UserTokenIndicator profile={profile} />
                    </div>
                )}
                 <div className="relative w-full">
                    <button onClick={() => setIsUserMenuOpen(!isUserMenuOpen)} className={`flex items-center gap-3 w-full p-2 rounded-lg hover:bg-slate-200 dark:hover:bg-slate-700 ${!isSidebarOpen && 'justify-center'}`}>
                        ...
                        {isSidebarOpen && <span className="text-sm font-medium text-slate-700 dark:text-slate-300 truncate">{user.email}</span>}
                    </button>
                    {isUserMenuOpen && (
                        <div ref={userMenuRef} className={`origin-bottom-left absolute bottom-full mb-2 w-56 rounded-md shadow-lg bg-white dark:bg-slate-800 ring-1 ring-black ring-opacity-5 py-1 z-30 ${isSidebarOpen ? 'left-0' : 'left-16'}`}>
                 ...
                 <button
                    ...
                    className="w-full h-10 flex items-center justify-center gap-2 mt-2 rounded-md text-sm font-semibold text-slate-500 dark:text-slate-400 hover:bg-slate-200 dark:hover:bg-slate-700"
